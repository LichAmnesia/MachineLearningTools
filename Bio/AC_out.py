...
# "D:\\Work\\bioinformatics\\hhsearch\\PDBCYS\\"
# filepath = "E:\\bioinformatics\\hhsbearch\\PDBCYS\\"
datafilepath = os.path.join(filepath, 'Database')
outfilepath = os.path.join(filepath, 'Output')
predictpath = os.path.join(filepath, 'Predictout')
...
def openfile(seqConfidentDic, filename):
    seqfile = open(os.path.join(datafilepath, filename), 'r')
    # hhrlist增加res
    scorefile = open(os.path.join(outfilepath, filename + '.score'), 'r')
    reslist = compare(seqConfidentDic, filename, scorefile, seqfile)
    seqfile.close()
    scorefile.close()


# 读取score文件到list[] ，每个是一个dic{'seqName': 'Q9Y6Y9', 'Name': 'Q60648',
# 'list': [(21, 132), (79, 89)], 'Num': 2, 'E-value': 1e-19, 'Identities':
# 17}
...
def readset(setId):
    setFile = open(os.path.join(setpath, 'set' + str(setId)))
    ...
    for setFileLine in setFile.readlines():
        if setFileLine[0] == '>':
            ...
            # 读取seqfile的信息
            seqfile = open(os.path.join(datafilepath, proteinName), 'r')
            seqfilelines = seqfile.readlines()
            seqNum = int(seqfilelines[0])
            seqSet = set([])
            for seqId in range(1, seqNum + 1):
...
def readpredictoutput(setId):
    predictFile = open(os.path.join(predictpath, 'output_' + str(setId)), 'r')
    predictOut = []
        ...
        if vis[i] == 0:
            for j in range(i + 1, num * 2):
                if vis[j] == 0:
                    ...
                    ret, rli = dfs(seqPredictOut, pairDicSeqBondNum,
                                   num, now + 1, vis, ans, li, ali)
                    if ans < ret:
                        ali = []
            ...
            for bondPair in seqConfidentDic[seqName]:
                ...
                for i in range(1, seqBondNum * 2 + 1):
                    ...
                    if tmpstr in pairDic[seqBondNum].keys() and i != a:
                        seqPredictOut[pairDic[seqBondNum][tmpstr]] -= 1
            isSeqTrue, seqTrueNum = solveseqpredict(
                seqBondNum, pairDic[seqBondNum], seqPredictOut)
            answerSeq += isSeqTrue
